...
import {
  ...
  getAllExchangeRates,
} from "@/src/actions/rates";
import { getInstitutions } from "@/src/actions/institutions";
import { Institution } from "@/types";
import { getExchangeRateClient } from "@/src/lib/exchange-rates-data";
...
export function usePreFetchInstitutions(
  ...
  return useQuery({
    ...
    staleTime: 60 * 1000, // 1 minute to pick up backend updates quickly
    refetchInterval: 3 * 60 * 1000, // refresh every 3 minutes in background
    gcTime: 10 * 60 * 1000,
    retry: 1,
    retryDelay: 1000,
    // Background fetch - light weight
    refetchOnWindowFocus: true,
    refetchOnReconnect: true,
  });
}
