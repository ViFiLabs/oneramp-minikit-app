...
import { Country, ExchangeRateResponse } from "@/types";
...
const SelectCountry = ({
  ...
  // Extract exchange rate for current country from map if provided, otherwise use prop or fetch
  const exchangeRateFromMap = useMemo(() => {
    if (exchangeRatesMap && country?.countryCode) {
      return exchangeRatesMap[country.countryCode];
    }
    return undefined;
  }, [exchangeRatesMap, country?.countryCode]);

  // Use provided exchange rates if available, otherwise fetch
  const { data: fetchedExchangeRate } = useExchangeRate({
    countryCode:
      exchangeRateProp || exchangeRateFromMap
        ? undefined
        : country?.countryCode,
    orderType: "selling",
    providerType:
  ...
  // Extract Nigeria rate from map if provided
  const nigeriaRateFromMap = useMemo(() => {
    if (exchangeRatesMap) {
      return exchangeRatesMap["NG"];
    }
    return undefined;
  }, [exchangeRatesMap]);

  // Use provided Nigeria rate if available, otherwise fetch
  const { data: fetchedNigeriaRate } = useExchangeRate({
    countryCode: nigeriaRateProp || nigeriaRateFromMap ? undefined : "NG",
    orderType: "selling",
    providerType:
      nigeriaRateProp || nigeriaRateFromMap
        ? undefined
        : paymentMethod || "momo",
  });

  // Use provided rates if available, otherwise use fetched data
  // Priority: prop > map > fetched
  const exchangeRate =
    exchangeRateProp || exchangeRateFromMap || fetchedExchangeRate;
  const nigeriaRate =
    nigeriaRateProp || nigeriaRateFromMap || fetchedNigeriaRate;
