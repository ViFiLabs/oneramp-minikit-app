...
const TransactionsModal = () => {
  ...
  // Fetch all transactions using the useFetchTx hook
  const {
    data: transactionsData,
    isLoading,
    error,
  } = useFetchTx(address || undefined, isConnected);

  // Extract transactions by status from the fetched data
  const processingTransactions: UITransaction[] =
    transactionsData?.[TransactionStatus.IN_PROGRESS]?.map(
      transformTransaction
    ) || [];

  const completedTransactions: UITransaction[] =
    transactionsData?.[TransactionStatus.DONE]?.map(transformTransaction) || [];

  const failedTransactions: UITransaction[] =
    transactionsData?.[TransactionStatus.FAILED]?.map(transformTransaction) ||
    [];
