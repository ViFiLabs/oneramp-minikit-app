...
import { getCNGNDefaultAmount } from "@/src/lib/exchange-rates-data";
...
import { FromPanel } from "@/src/components/panels/FromPanel";
import { SwapArrow } from "@/src/components/panels/SwapArrow";
import { SwapHeader } from "@/src/components/panels/SwapHeader";
...
export function WithdrawInterface({
  ...
  // Get payment method from user selection store for exchange rates
  const paymentMethod = userSelectionStore.paymentMethod || "momo";
  ...
  // For withdraw, amount is in crypto (USDC/cNGN), so we compare directly
  const exceedsBalance = useMemo(() => {
    if (!amount || !evmConnected) return false;
    if (isBalanceLoading || currentBalance === "...") return false;
  ...
  }, [amount, currentBalance, isBalanceLoading, evmConnected]);
